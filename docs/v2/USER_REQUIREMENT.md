...
| SR_21 | 작업 큐 관리 기능 | 시스템은 저장된 루틴을 순서대로 줄 세워 연속 실행한다. 작업 1개는 `Recordings/*.json` 루틴 1개다. 항목별 반복 횟수·사용 여부·실행 상태(대기/진행/완료/실패/건너뜀)를 관리한다. 항목 추가·삭제·순서 변경이 가능하다. 다음 항목은 이전 재생이 실제로 끝난 뒤에 시작한다. 큐는 `Recordings/Queues/*.json` 에 저장한다. 큐 실행 중에는 관절 입력을 막는다. 상세 명세는 `docs/TASK_QUEUE.md`. 2026-08-04 구현 (`TaskQueueRunner` 외 2파일). | O | UR_13 |
...
## **Scenario #8 — 작업 큐 연속 운전** (2026-08-04 구현, 2026-08-06 실물 검증)

1. **사용자가 Recorder 를 열고 제목줄 스위치를 「시나리오」 로 바꾼 뒤 「항목 추가」 를 누른다.**
...
| 4-7 | 협동 작업 (SR_19) | Recorder + 작업 큐 | ✅ 전용 모드가 아니라 루틴 스텝의 대상 지정(R1/R2/둘 다)으로 만든다. 2026-08-06 두 로봇 전달까지 실물 확인. `ControlMode` 가 2개뿐인 것은 그대로이고 `PROJECT_NOTES.md` 의 5모드 기술은 🔺 **구버전** |
