...
```bash
# 서버 코드와 기동 스크립트를 라즈베리파이에 복사
scp raspberry_pi/robot_server_dual.py sw@<라파IP>:/home/sw/
scp raspberry_pi/start_server.sh      sw@<라파IP>:/home/sw/
ssh sw@<라파IP> "chmod +x /home/sw/start_server.sh"
```

> ⚠️ 라즈베리파이 IP는 자주 바뀝니다. 라파에서 `hostname -I` 로 확인하세요.
> 접속이 안 되면 **PC와 같은 대역인지** 먼저 보세요.
> 다른 공유기에 붙어 있으면 라우팅이 없어 IP를 맞게 써도 닿지 않습니다.
...
### 스크립트 추가
`Assets/Script/` 폴더의 `.cs` 파일 전체를 Unity 프로젝트로 복사:

```
SOArmJointConfig.cs      SOArmPresets.cs
ISOArmController.cs      SOArmMotorMapper.cs
SOArmSocketClient.cs     SOArmSimController.cs
SOArmRealController.cs   SOArmManager.cs
SOArmDualManager.cs      SOArmMotionRecorder.cs
PincOpenCoupling.cs      PincOpenSafety.cs
SmartFactoryUI_v3_4.cs
```

`Assets/Editor/` 의 에디터 도구도 함께 복사하면 그리퍼 설정·검증이 편합니다.

### 씬 구성

#### 1. SmartFactoryManager (빈 GO)
- `SOArmDualManager` 컴포넌트 추가
- `SmartFactoryUI_v3_4` 컴포넌트 추가

#### 2. Robot1_Group (빈 GO)
- `SOArmSocketClient` 추가
  - Server IP: 라즈베리파이 IP (`hostname -I` 로 확인)
  - Server Port: `5000`
...
```bash
ssh sw@<라파IP>
bash /home/sw/start_server.sh      # 기동 + 로그·포트 확인까지 한 번에
```
...
> ⚠️ 서버를 다시 띄울 때는 **`pkill -9`** 로 죽여야 합니다.
> 기본 `pkill`(SIGTERM)은 종료 핸들러를 깨워 `disable_torque()` 를 호출하고,
> 그러면 **12V 팔이 중력으로 주저앉습니다.** `start_server.sh` 는 `-9` 를 씁니다.

### 2. Unity Play

⚠️ 먼저 **Player Settings → Run In Background 를 켜세요.**
꺼져 있으면 창이 비활성일 때 Editor가 멈춰서 통신이 끊긴 것처럼 보입니다.

1. Unity에서 **▶ Play 버튼** 클릭
2. Game 화면에 제어 UI 표시 확인
3. 슬라이더를 움직이면 시뮬과 실제 로봇이 함께 움직임

> Play 직후에는 실물 자세를 한 번 읽기 전까지 명령을 내보내지 않습니다.
> **실물이 진실이고 화면이 따라오는** 방향이라 정상 동작입니다.
...
다음 단계는 [README](../README.md) 의 **직접교시 사용법** 을 참고하세요.
손으로 팔을 움직여 동작을 가르치고 녹화·재생할 수 있습니다.
