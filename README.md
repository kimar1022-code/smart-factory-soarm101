...
유니티 쪽 `teachMode`도 함께 켜진다. 안 켜면 옛 목표가 계속 나가서
...
- [x] 카티시안 좌표 제어 (역기구학 IK) — `SR_20` / `FR-38`. 라파의 placo 로 풀고 유니티가 적용한다
...
2026-08-04 부터 홈은 **접힌 자세 `(0, -90, 64, -80, 0)`** 다 (TCP `(0.039, 0, 0.364)`).
그 전에는 전관절 0° 였는데, 팔을 앞으로 쭉 뻗은 자세라 켤 때마다 책상 앞을 크게 쓸었다.
...
```
SO-ARM → 홈 포즈 적용 (접힌 자세)     그다음 씬 저장 (Ctrl+S)
```

### 회전 조그 (Rx / Ry / Rz)

5축이라 위치와 자세를 동시에 다 맞출 수 없다. 회전 요청으로 TCP 가 5mm 넘게 밀리면
서버가 결과를 버리고 `blocked_by: rot_drift` 로 거절한다.

| 축 | 동작 |
|---|---|
| `Rz` | 어느 자세에서나 안정적 |
| `Ry` | 대체로 되지만 뻗은 자세에서 한계에 걸린 사례 있음 (실측 5.26mm) |
| `Rx` | **자세를 심하게 탄다.** 아래 참고 |

`Rx` 회전은 J1(베이스 yaw)로 만들어진다. TCP 가 베이스 축 위에 있으면 J1 을 돌려도
TCP 가 제자리에서 돌아 통과하지만, 팔이 조금만 펴지면 TCP 가 호를 그리며 크게 밀린다.
...
즉 홈 근처에서만 되고, 실제로 작업할 만한 자세에서는 사실상 항상 막힌다.
...
- 카티시안 UI 에 `Rx` 버튼이 그대로 있다. 대부분 거절당하므로 지우거나 비활성 표시할지 미정
