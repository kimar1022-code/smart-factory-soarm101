...
namespace SOArmControl
{
    ...
    public class SmartFactoryUI_v3_4 : MonoBehaviour
    {
        ...
        void DrawTopBar()
        {
            ...
            int dx = w + 5;
            if (GUI.Button(new Rect(x + dx * 0, y, w, h), "Robot1만"))
                dualManager.ChangeMode(SOArmDualManager.ControlMode.Robot1Only);
            if (GUI.Button(new Rect(x + dx * 1, y, w, h), "Robot2만"))
                dualManager.ChangeMode(SOArmDualManager.ControlMode.Robot2Only);
            if (GUI.Button(new Rect(x + dx * 2, y, w, h), "독립"))
        ...
        void DrawBottomBar()
        {
            ...
            if (GUI.Button(new Rect(x + btnW + gap, y, btnW, btnH), "⏸ 전체 정지"))
                dualManager.StopAll();
